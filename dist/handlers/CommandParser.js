export class CommandParser {
    ...
    suggestCommand(input) {
        ...
        if (words.some(w => ['show', 'get', 'list'].includes(w))) {
            suggestions.push('show pipelines for [application]', 'show pipeline [id] for [application]', 'show snapshots for [application]', 'list deploys for [application] in [environment]');
        }
        if (words.some(w => ['run', 'execute', 'start'].includes(w))) {
            suggestions.push('run pipeline [id] for [application]', 'execute pipeline [id] in [application] with {"param": "value"}');
        }
        if (words.some(w => ['deploy', 'snapshot'].includes(w))) {
            suggestions.push('deploy snapshot [id] to [application] using [pipeline]', 'get snapshot for [application] with [sha] [branch]');
        }
        return suggestions;
    }
}
